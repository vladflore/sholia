...
class ItemControllerTest {
	...
	void fetchAllItems(String name) throws Exception {
		mockMvc.perform(get("/api/v1/items?name=" + name)).andExpect(status().isOk())
	...
	void createItem() throws Exception {
		...
		BDDMockito.when(itemService.save(any(ItemDto.class))).thenReturn(savedItem);

		mockMvc.perform(post("/api/v1/items").contentType(MediaType.APPLICATION_JSON).content(requestBody))
				.andExpect(status().isOk()).andExpect(jsonPath("$.id", Is.is(1)))
				.andExpect(jsonPath("$.name", Is.is("apples")));
